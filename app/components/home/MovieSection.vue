<template>
  <section class="py-6">
    ...
    <!-- Detail Preview Modal (카드 클릭 시 열림, 데이터 로딩 중엔 스켈레톤) -->
    <Transition name="modal">
      ...
      >
        ...
        <div class="relative w-full max-w-4xl glass-dark rounded-2xl overflow-hidden animate-fade-in-up z-10">
          <!-- 로딩 스켈레톤: 실제 콘텐츠와 동일한 높이/너비 -->
          <template v-if="!detailData">
            <CommonSkeletonLoader variant="custom" class="w-full h-48 md:h-64 rounded-none" />
            <div class="p-6 -mt-20 relative flex gap-4 md:gap-6">
              <CommonSkeletonLoader variant="custom" class="hidden md:block w-32 lg:w-40 aspect-[2/3] rounded-xl shrink-0" />
              <div class="flex-1 min-w-0 space-y-3">
                <CommonSkeletonLoader variant="custom" class="h-7 w-3/4 max-w-xs" />
                <CommonSkeletonLoader variant="custom" class="h-3 w-full" />
                <CommonSkeletonLoader variant="custom" class="h-3 w-full" />
                <CommonSkeletonLoader variant="custom" class="h-3 w-4/5" />
                <CommonSkeletonLoader variant="custom" class="h-10 w-28 mt-4 rounded-lg" />
              </div>
            </div>
          </template>
          <!-- 데이터 로드 완료 -->
